...
O comando usa a configuração do Dependency Cruiser e grava o resultado em `dependency-graph.svg` na raiz do projeto. A saída JSON do `depcruise` é convertida para um grafo DOT/SVG via [`viz.js`](https://www.npmjs.com/package/viz.js), pulando os arquivos `index.ts` usados como *barrel*, ignorando completamente arquivos de teste (`*.spec.ts`/`*.test.ts`) e conectando diretamente os módulos que eles reexportam. O grafo agrupa os nós pela primeira pasta dentro de `src` e usa rotas ortogonais com espaçamento maior entre ranks/nós para diminuir o entrelaçamento das arestas. Ao abrir o SVG no navegador, passar o mouse ou focar em um nó evidencia suas dependências (arestas/nós vizinhos) e atenua os demais elementos para facilitar a leitura visual.
